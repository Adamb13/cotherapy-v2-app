...
// Sample moments that would be extracted by AI
const SAMPLE_EXTRACTED_MOMENTS = [
  {
    category: 'INSIGHT',
    content: 'Client recognized pattern of seeking external validation before making decisions, connecting it to childhood experiences with critical parent.',
    ai_significance: 4
  },
  {
    category: 'EMOTIONAL_SHIFT',
    content: 'Noticeable shift from frustration to sadness when discussing relationship with sister. Tears present but client remained regulated.',
    ai_significance: 3
  },
  {
    category: 'PARTS_WORK',
    content: 'Identified a "perfectionist manager" part that fears judgment. Client able to access curiosity toward this part briefly.',
    ai_significance: 5
  },
  {
    category: 'RESISTANCE',
    content: "Client deflected when approaching topic of father's illness. Changed subject twice. May indicate protective part activation.",
    ai_significance: 3
  }
]

const SAMPLE_EXTRACTED_KTMS = [
  {
    content: 'You noticed that seeking validation is a pattern, not a character flaw. That awareness itself is progress.',
    ai_emphasis: 4
  },
  {
    content: "The perfectionist part is trying to protect you from judgment. It's been working hard for a long time.",
    ai_emphasis: 5
  }
]
...
export default function PostSession({ therapist, client, onNext }) {
  ...
  async function handleAnalyze() {
    // Simulate AI analysis - in real app, call Claude API
    const extractedMoments = SAMPLE_EXTRACTED_MOMENTS.map(m => ({
      ...m,
    ...
    const extractedKtms = SAMPLE_EXTRACTED_KTMS.map(k => ({
      ...k,
      id: crypto.randomUUID(),
      session_id: session?.id,
      client_id: DEMO_CLIENT_ID,
    ...
    setMoments(extractedMoments)
    setKtms(extractedKtms)
    setStep(1)
  }
        ...
        <>
          <div className="flex justify-between items-center mb-24">
            <div>
              ...
              <p style={{ color: 'var(--warm-gray)', fontSize: 14 }}>
                AI-proposed messages for intersession reinforcement. Edit wording to match your voice.
              </p>
            </div>
          </div>
              ...
              >
                ...
                {ktm.approved === null && (
                  <div className="moment-actions">
                    ...
                    <button className="btn secondary small">Edit Wording</button>
                    <button 
                      className="btn ghost small"
          ...
          <button className="btn secondary mt-16">+ Add Custom KTM</button>
